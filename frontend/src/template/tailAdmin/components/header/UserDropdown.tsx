...
function deriveDisplayName(user?: TailAdminUser): string {
  if (user?.name && user.name.trim()) return user.name;
  if (user?.email && user.email.includes("@")) return user.email.split("@")[0] ?? "User";
  return "User";
}

function deriveInitials(user?: TailAdminUser): string {
...
export default function UserDropdown({
  ...
  const isLoggedIn = Boolean(user?.email || user?.name);
  ...
  async function handleSignOut() {
    ...
    try {
      ...
      navigate(signOutRedirectTo, { replace: true });
    }
  }
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          {hasAvatar ? (
            ...
            <span className="flex h-full w-full items-center justify-center bg-gray-100 text-gray-700 text-theme-sm font-semibold dark:bg-gray-800 dark:text-gray-200">
              {initials}
            </span>
          )}
        </span>
