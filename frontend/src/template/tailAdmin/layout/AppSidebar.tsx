...
const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  const hasOthersSection = othersItems.some(
    (item) => Boolean(item.path) || Boolean(item.subItems?.length)
  );
      ...
      {items.map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            >
              <span
                className={`menu-item-icon-size inline-flex items-center justify-center ${
                  openSubmenu?.type === menuType && openSubmenu?.index === index
    ...
    >
      ...
      >
        <Link to={branding?.href ?? "/"}>
          {isExpanded || isHovered || isMobileOpen ? (
            branding?.expandedLightSrc && branding?.expandedDarkSrc ? (
              <>
                <img
              ...
              <div className="h-10 w-[150px] rounded-lg bg-gray-100 dark:bg-white/[0.03]" />
            )
          ) : branding?.collapsedSrc ? (
            <img
            ...
            <div className="h-8 w-8 rounded-lg bg-gray-100 dark:bg-white/[0.03]" />
          )}
        </Link>
      </div>
