...
const ROW_STYLE: Record<string, string> = {
  Fatal:   'border-l-2 border-purple-400 bg-purple-50/30',
  Error:   'border-l-2 border-red-400 bg-red-50/30',
  Warning: 'border-l-2 border-amber-400 bg-amber-50/30',
};
              ...
              <tbody>
                @for (entry of entries(); track $index) {
                  <tr class="border-t border-gray-50 hover:bg-gray-50/50 transition"
                    ...
                    <td class="px-4 py-1.5">
                      <div class="text-gray-800 break-all">{{ entry.message }}</div>
                      @if (entry.exception) {
                        <details class="mt-0.5">
                          <summary class="cursor-pointer text-red-600 hover:text-red-800 text-[10px]">Exception ▼</summary>
                          <pre class="mt-1 whitespace-pre-wrap text-red-700 text-[10px] bg-red-50 rounded p-2 max-h-40 overflow-y-auto">{{ entry.exception }}</pre>
                        </details>
                      }
                    </td>
                  </tr>
...
export class LogViewerComponent implements OnInit {
  ...
  levelStyle(level: string): string {
    return LEVEL_STYLE[level] ?? 'bg-gray-100 text-gray-600';
  }
