          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            @if (template()) {
              <button type="button" (click)="toggleFavourite()"
                ...
                [class]="template()!.is_favourite ? 'text-yellow-400 hover:text-yellow-500' : 'text-gray-400 hover:text-yellow-400'"
                [title]="template()!.is_favourite ? 'Remove from favourites' : 'Add to favourites'">
                <svg class="w-5 h-5" [attr.fill]="template()!.is_favourite ? 'currentColor' : 'none'" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if (template() && viewMode() === 'table') {
              ...
              <a [href]="excelUrl()" class="ta-btn ta-btn-secondary text-sm" target="_blank" rel="noopener" title="Export as Excel (.xlsx)">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class ReportViewerComponent implements OnInit {
  ...
  excelUrl(): string {
    return this.reportService.exportExcelUrl(
      this.template()!.id,
      this.sortField(),
      this.sortDirection(),
      this.runtimeFilters(),
  ...
  toggleFavourite(): void {
    ...
    if (t.is_favourite) {
      this.reportService.removeFavourite(t.id).subscribe({
        next: () => this.template.update(x => x ? { ...x, is_favourite: false } : x),
      });
    } else {
      this.reportService.addFavourite(t.id).subscribe({
        next: () => this.template.update(x => x ? { ...x, is_favourite: true } : x),
      });
    }
  }
