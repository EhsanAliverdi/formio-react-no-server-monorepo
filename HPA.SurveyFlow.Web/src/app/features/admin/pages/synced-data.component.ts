    ...
    <div class="p-6">
      ...
      <div class="mb-6 bg-white rounded-xl border border-gray-200 p-4">
        <div class="flex items-start gap-3">
          <div class="flex-1">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Sync Asset by ID</label>
            <p class="text-xs text-gray-500 mb-2">Enter an external asset ID to fetch it and all its parent assets directly from the integration.</p>
            <div class="flex items-center gap-2">
              <input type="text" [(ngModel)]="syncOneId" placeholder="e.g. 10939"
                (keydown.enter)="syncOne()"
                class="w-48 rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
          ...
          @if (syncOneResult()) {
            <div class="flex-1 rounded-lg border p-3 text-xs"
              [class]="syncOneResult()!.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'">
              <div class="font-semibold mb-1"
                [class]="syncOneResult()!.success ? 'text-green-800' : 'text-red-800'">
      ...
      <div class="mb-4 bg-white rounded-xl border border-gray-200 p-4">
        <div class="flex items-center gap-4">
          <div class="flex-1">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Best-Effort Gap Fill</label>
      ...
      @if (sources().length > 0) {
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          @for (s of sources(); track s.source) {
            <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-start gap-3 cursor-pointer hover:border-indigo-300 transition"
              [class]="filterSource === s.source ? 'border-indigo-400 bg-indigo-50' : ''"
      ...
      <div class="flex flex-wrap gap-3 mb-4">
        ...
        <button type="button" (click)="expandAll()"
          class="px-3 py-1.5 text-xs rounded-lg border border-gray-300 hover:bg-gray-50 transition">
          Expand all
        </button>
        ...
        <div class="text-sm text-gray-500 self-center ml-auto">
          ...
          <table class="min-w-full text-sm">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left font-medium text-gray-500 w-28">ID</th>
                <th class="px-4 py-3 text-left font-medium text-gray-500 w-36">Category</th>
                <th class="px-4 py-3 text-left font-medium text-gray-500 w-28">Location</th>
                <th class="px-4 py-3 text-left font-medium text-gray-500 w-20">Status</th>
              ...
              @if (loading()) {
                ...
                @for (node of visibleNodes(); track node.id) {
                  <tr class="border-t border-gray-100 hover:bg-gray-50/70 transition"
                    ...
                    <td class="px-4 py-2 font-mono text-xs text-gray-500">{{ node.external_id }}</td>
                    <td class="px-4 py-2 text-gray-600 text-xs">{{ node.category ?? '—' }}</td>
                    <td class="px-4 py-2 text-gray-600 text-xs">{{ node.location ?? '—' }}</td>
                    <td class="px-4 py-2">
                      @if (node.is_active) {
                    ...
                    <td class="px-4 py-2">
                      <button type="button" (click)="openDetail(node)"
                        class="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 transition">
                        Details
                      </button>
                    </td>
                  </tr>
      ...
      @if (selectedAsset()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div class="w-full max-w-3xl max-h-[90vh] flex flex-col rounded-xl bg-white shadow-xl overflow-hidden">
            <div class="flex items-start justify-between px-6 py-4 border-b border-gray-200 shrink-0">
              <div class="min-w-0">
                <h3 class="text-lg font-semibold text-gray-900">{{ selectedAsset()!.display_name }}</h3>
            ...
            <div class="flex border-b border-gray-200 bg-gray-50 shrink-0">
              <button type="button" (click)="detailTab = 'overview'"
                [class]="detailTab === 'overview'
                  ? 'px-5 py-3 text-sm font-semibold text-indigo-700 border-b-2 border-indigo-600 bg-white'
                  : 'px-5 py-3 text-sm text-gray-600 hover:text-gray-900'">
                Overview
              </button>
            ...
            <div class="flex-1 overflow-y-auto">
              @if (rawLoading()) {
                      ...
                      <div class="px-6 py-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                          @for (field of group.fields; track field.label) {
                            <div>
                              <dt class="text-xs text-gray-500 mb-0.5">{{ field.label }}</dt>
                              @if (field.type === 'multiline') {
                                <dd class="text-sm text-gray-800 whitespace-pre-wrap">{{ field.value }}</dd>
                                ...
                                <dd class="text-sm text-gray-800 font-medium">{{ field.value }}</dd>
                              }
                            </div>
                          }
                        </div>
                ...
                <div class="p-6">
                  <pre class="text-xs text-gray-700 bg-gray-50 border border-gray-200 rounded-lg p-4 overflow-x-auto whitespace-pre-wrap leading-relaxed">{{ rawJson() }}</pre>
                </div>
              }
            </div>
            ...
            <div class="px-6 py-3 border-t border-gray-200 flex items-center justify-between shrink-0">
              ...
              <div class="flex items-center gap-2">
                ...
                <button type="button" (click)="selectedAsset.set(null)"
                  class="px-4 py-2 text-sm rounded-lg border border-gray-300 hover:bg-gray-50 transition">Close</button>
              </div>
            </div>
          </div>
        </div>
