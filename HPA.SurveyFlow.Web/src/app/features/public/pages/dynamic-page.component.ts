...
import { Page, SiteSettings } from '../../../core/models';
...
@Component({
  ...
  template: `
    @if (loading()) {
      <div class="min-h-[60vh] py-20 text-center text-sm text-gray-400">Loading page...</div>
    } @else if (error()) {
      <div class="mx-auto max-w-2xl px-6 py-20 text-center">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Page unavailable</h1>
        <p class="mt-3 text-gray-500 dark:text-gray-400">{{ error() }}</p>
        <a routerLink="/" class="ta-btn ta-btn-primary mt-6">Go Home</a>
      </div>
    } @else {
      @if (page()?.use_layout) {
        <app-layout
          ...
          (signOut)="logout()"
        >
          <article class="dynamic-page dynamic-page-framed">
  ...
  `,
  styles: [`
    .dynamic-page { min-height: 60vh; background: #fff; color: #111827; }
...
export class DynamicPageComponent implements OnInit {
  ...
  headerUser = computed<HeaderUser | undefined>(() => {
    ...
    const name = (u.preferred_name ?? u.display_name ?? '').trim()
      ...
      || u.email;
    return { name: name || u.email, email: u.email, avatarUrl: typeof u.avatar_url === 'string' ? u.avatar_url : undefined };
  });
  ...
  safeDocument = computed<SafeHtml>(() => {
    ...
    return this.sanitizer.bypassSecurityTrustHtml(`<style>${page?.css || ''}</style>${page?.html || ''}`);
  });
  ...
  ngOnInit(): void {
    ...
    this.pageService.getBySlug(slug).subscribe({
      ...
      error: err => {
        this.error.set(err?.status === 401 ? 'Sign in to view this page.' : 'The requested page could not be found.');
