...
@Component({
  ...
  styles: [`
    ...
    .card-overlay { transition: opacity 0.35s ease; opacity: 0; }
    .card-root:hover .card-overlay { opacity: 1; }
    .card-content {
      transition: transform 0.35s cubic-bezier(0.34,1.2,0.64,1), opacity 0.3s ease;
      transform: translateY(12px); opacity: 0;
    }
    .card-root:hover .card-content { transform: translateY(0); opacity: 1; }
    .card-btn {
      transition: background 0.15s ease, transform 0.15s ease, box-shadow 0.15s ease;
    }
    .card-btn:hover { transform: scale(1.03); box-shadow: 0 4px 16px rgba(37,99,235,0.45); }
    .card-btn:active { transform: scale(0.97); }

    /* Preview mode: disable hover effects */
    :host(.preview-mode) .card-root:hover { transform: none; box-shadow: none; }
  ...
  `],
  template: `
    <!-- ── Overlay style (current hover-reveal) ── -->
      ...
      <div class="card-root relative block w-full rounded-2xl overflow-hidden shadow-md cursor-pointer select-none"
        ...
        @if (imageUrl) {
          ...
          <div class="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
            <img [src]="iconSvgUrl" [alt]="name" class="card-img w-28 h-28 object-contain" />
          </div>
        } @else {
          <div class="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
      ...
      <div class="card-root block w-full rounded-2xl overflow-hidden shadow-md cursor-pointer select-none bg-white border border-gray-100">
        ...
        <div class="relative overflow-hidden" style="aspect-ratio: 16/9;">
          @if (imageUrl) {
            ...
            <div class="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
              <img [src]="iconSvgUrl" [alt]="name" class="card-img w-20 h-20 object-contain" />
            </div>
          } @else {
            <div class="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
              <svg class="card-img w-16 h-16 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                  d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
...
export class FormCardComponent {
  ...
  @Input() cardStyle: 'overlay' | 'compact' = 'overlay';
}
